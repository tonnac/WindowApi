...
bool RObject::Frame()
{
	...
	{
		m_CenterPos.x += (-1 * g_fPerSecFrame* m_CenterPos.x);
	}
	if (!isFixed)
	{
	...
	if (m_fZoom > 1.0f)
	{
		getRotateBitmap(m_hbColorBitmap, m_ColorBitmap, m_fZoom);
		getRotateBitmap(m_hbMaskBitmap, m_MaskBitmap, m_fZoom);
	}
	else
	{
...
void RObject::SetDegree(FLOAT fAngle)
{
	m_fAngle = fAngle;
	isFixed = true;
}
...
void RObject::getRotateBitmap(HBITMAP hbit, Bitmap* pBitmap, float x)
{
	FLOAT fRadian = static_cast<FLOAT>(DegreeToRadian(m_fAngle));
	FLOAT Cosine = cos(fRadian);
	FLOAT Sine = sin(fRadian);
